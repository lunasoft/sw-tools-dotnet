...
namespace SW.Tools.Services.Sign
{
    internal class SignService
    {
        ...
        internal static string SignRetencion(byte[] pfx, string password, string xml, string version = "2.0")
        {
            try
            {
                X509Certificate2 x509Certificate = new X509Certificate2(pfx, password
                ...
                var nCertificate = SignUtils.CertificateNumber(x509Certificate);
                var certificate = Convert.ToBase64String(x509Certificate.GetRawCertData());
                xml = SignUtils.SignXml(xml, nCertificate, certificate, version != "2.0" ? true : false);
                var originalString = GetCadenaOriginalRetencion(xml, version);
                var signResult = SignUtils.GetSignature(password, pfx, originalString, version != "2.0" ? "SHA1" : "SHA256");
        ...
        internal static string GetCadenaOriginalRetencion(string xml, string version = "2.0")
        {
            try
            {
                var xslt_cadenaoriginal = new XslCompiledTransform();
                if(version != "2.0")
                {
                    xslt_cadenaoriginal.Load(typeof(cadenaoriginal_retenciones_1_0));
                }
                else
                {
